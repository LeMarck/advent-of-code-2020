...
class Password {
  ...
  Password(String line) {
    RegExpMatch match = new RegExp(r'^(\d+?)-(\d+?) (.): (.+)$').firstMatch(line);

    this.min = int.parse(match.group(1));
    this.max = int.parse(match.group(2));
    this.char = match.group(3);
    this.password = match.group(4);
  }
}

List<Password> passwords = data
    .split(new RegExp(r'\n'))
    .where((element) => element != '')
    .map((element) => new Password(element))
    .toList();
